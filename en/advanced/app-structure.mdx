...
```mermaid
flowchart TD
    A["public/index.php<br>Entry point"] --> B["Requires bootstrap/app.php<br>to obtain the Application"]
    B --> C["Application::configure()<br>Creates ApplicationBuilder"]
    C --> D["withKernels()<br>Registers HTTP/Console kernels"]
    D --> E["withEvents()<br>Configures event discovery"]
    E --> F["withCommands()<br>Sets paths for Artisan commands"]
    F --> G["withProviders()<br>Loads bootstrap/providers.php"]
    G --> H["withRouting()<br>Configures routing"]
    H --> I["withMiddleware()<br>Configures middleware"]
    I --> J["withExceptions()<br>Configures exception handling"]
    J --> K["create()<br>Returns the Application instance"]
    K --> L["HTTP Kernel processes the request<br>Middleware → Router → Controller"]

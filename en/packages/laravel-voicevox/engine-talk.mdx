...
Start your app with `php artisan serve`, then use endpoints like `/audio_query` and `/synthesis` directly.
...
```shell
curl -s -X POST "http://127.0.0.1:8000/audio_query?speaker=1&text=I%20love%20Laravel" \

...
Tags let you group related cache items and flush them together. Tags require a driver that supports them (Redis or Memcached). Tags are not available with the `file`, `dynamodb`, `database`, or `storage` drivers:

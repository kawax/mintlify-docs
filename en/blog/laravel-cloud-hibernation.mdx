...
On modern Flex compute, the faster **Scale to Zero** model is now the successor.
...
<Info>
  Legacy hibernation pauses scheduler and queue processing while asleep. Scale to Zero resolves this in most cases, and queue processing is most reliable when combined with Managed Queues so app sleep does not block job handling.
</Info>

...
[Compute](https://cloud.laravel.com/docs/compute#scale-to-zero) explains that **Flex** is the low-cost, flexible compute class. The new **Scale to Zero** evolved from legacy hibernation on Flex compute. With the new **Flex compute**, cold starts on HTTP requests are now **under 500ms**, instead of the older 5–20 second behavior.
...
Running `schedule:run` on the App cluster while delegating jobs to Managed Queues is now the most practical default architecture in the official docs.

<Info>
  A self-managed `queue:work` process on the App cluster can still be interrupted across sleep timeouts. If reliable queue continuity matters, prefer Managed Queues as documented.
</Info>

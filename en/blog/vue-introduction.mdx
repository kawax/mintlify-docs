...
The essential template directives you need to read and write Vue starter kit code.

#### `{{ }}` — Interpolation

Use double curly braces to embed JavaScript values or expressions in the template.
...
<template>
    ...
    <p v-else-if="role === 'admin'">Logged in as administrator</p>
    <a v-else href="/login">Log in</a>
</template>
```

Equivalent to Svelte's `{#if}` or React's ternary expressions.
...
Always include `:key` for efficient DOM diffing — equivalent to React's `Array.map()` with a `key` prop.

#### `v-model` — Two-way binding

`v-model` synchronises a form element with a reactive variable in both directions.
...
- `:attr="value"` — binds a dynamic value to an HTML attribute (`v-bind:attr` shorthand)
- `@event="handler"` — attaches an event listener (`v-on:event` shorthand)
...
The starter kit uses [Wayfinder](/en/blog/wayfinder-introduction) to manage routes as objects. `store.form()` returns an object containing `action` and `method` which is spread onto `<Form>` via `v-bind`.

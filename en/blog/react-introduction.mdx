...
React uses JSX — an HTML-like syntax inside JavaScript. These are the patterns you'll encounter most often in the starter kit.

#### `{}` — Interpolation

Use curly braces to embed JavaScript values or expressions in JSX.
...
React has no dedicated `v-if` directive. Use JavaScript's `&&` operator for simple conditions and the ternary operator `? :` for if/else branches.
...
// Nested condition (if / else if / else)
{isLoggedIn
    ? <p>Welcome back!</p>
    : role === 'admin'
...
Use `Array.map()` to render lists. Always include the `key` prop for efficient DOM diffing.
...
Because JSX compiles to JavaScript, `class` is a reserved keyword. Use `className` for CSS class names.
...
Event handler attributes in JSX use camelCase and accept a function reference.
...
The `<Form>` component from `@inertiajs/react` is the recommended form-submission style used in the starter kit's authentication pages. You specify `action` and `method` as props and receive `errors` and `processing` via a render-prop children function.
...
The children of `<Form>` is a function `({ errors, processing }) => JSX` — a render prop pattern. The `Form` component computes and injects `errors` and `processing` automatically. Form fields use the native `name` attribute instead of controlled `onChange` handlers, so the browser's standard form data collection handles serialisation.

### Starter kit pattern

The starter kit uses [Wayfinder](/en/blog/wayfinder-introduction) to manage routes as objects. `store.form()` returns an object containing `action` and `method` which is spread onto `<Form>`.

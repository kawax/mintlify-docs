...
Use `charge()` for one-time billing. Pass the amount in the lowest currency denomination (for example, `100` means `$1.00` in USD).
...
To generate downloadable PDFs, install `dompdf/dompdf` and call `downloadInvoice()`.

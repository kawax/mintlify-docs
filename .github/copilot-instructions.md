...
- https://github.com/inertiajs/docs Inertiaは個別のdocsリポジトリ。ここもMintlify。
...
- 中級（深く掘り下げる/Digging Deeper）：[公式ドキュメント](https://github.com/laravel/docs) の範囲のガイド
- 上級（上級者向けガイド/Advanced Guides）：[GitHub](https://github.com/laravel) のコードまで見ないと分からない機能のガイド。フレームワーク本体だけでなく公式パッケージも対象。
- ブログ：機能ごとに分けられない記事。
...
- `index.mdx`は言語別に主なページへのリンク

## Languages

管理者がレビューできる日本語をプライマリーにする。英語はセカンダリー。その他の言語は可能なら追加。

ページが増えた後は、最初に表示するのは英語メインにして多言語版は切り替えて表示する形式。ナビゲーションも多言語対応できるはず。

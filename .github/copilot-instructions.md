...
- 初級（チュートリアル/Tutorial）：基本的なチュートリアルとガイド。https://laravel.com/docs のGetting StartedやThe Basics、Eloquent: Getting Started辺りが初級。最初から必ず知っておきたいこと。残りは中級の範囲。
...
- ブログ：機能ごとに分けられない記事。
  ...
  - advancedとの違い。blogはLaravelのバージョンアップで変わる内容は作成日時点の情報で書いて更新しない。アップグレードガイドや新リポジトリ初期調査は更新しない方針で大丈夫。プロダクトや環境構築はバージョンアップとは関係ないので更新する。パッケージ紹介が曖昧だけどおそらく更新する。
- マイパッケージ/My packages：管理者が開発してるパッケージの解説ページ。専用のドキュメントサイトは作ってないのでここで一緒に入れて公式ドキュメントとする。ファイルは`jp/packages/`に作る。Mintlifyのテーマを変更してカテゴリーを増やしても見やすくなったので独立したカテゴリーにする。ブログの後ろに配置。
...
タブの並び順は変えるかもしれない。独自コンテンツを先に表示したくなったら変える。→パッケージを最初に移動。

- config/navigation-jp.json
  - config/jp/navigation-tutorial.json 
  - config/jp/navigation-guide.json 
  - config/jp/navigation-advanced.json 
  - config/jp/navigation-blog.json 
  - config/jp/navigation-packages.json 
- config/navigation-en.json
  - config/en/navigation-tutorial.json
  - config/en/navigation-guide.json
  - config/en/navigation-advanced.json
  - config/en/navigation-blog.json
  - config/en/navigation-packages.json

新しいページは基本的にはグループの一番下に追加。

## 執筆規約

コンテンツがなにもない時の初期規約なのでここはあまり拘らなくていい。

- アクティブボイス、二人称で書く（「あなたは」「~してください」など）。英語ならYouが必要だけど日本語で「あなた」を多用するとくどいので控えめにする。
- 1つの文には1つのアイデアのみ

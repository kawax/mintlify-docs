...
- [ ] Laravel12期間の途中からLaravelでもAI対応が進んでいるので重要。`ai.md`は初級。`boost.md` `mcp.md` `ai-sdk.md`の個別パッケージの詳細は中級。「Boostのカスタムエージェントを作る」「LaravelでMCPサーバーを作る」「AI SDKのカスタムプロバイダーを作る（公式ドキュメントにもまだない）」などの内容は上級で扱う。

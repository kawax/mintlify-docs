...
- [ ] `jp/tutorial/installation.mdx`は`スターターキット（Breeze、Jetstream）`もモデルの古い知識で書いて間違っていた。Laravel12リリースと同時にBreeze、Jetstreamではなく完全に新しいスターターキットが導入されている。詳細はdocsの`starter-kits.md`

...
Amazon SQS はメッセージペイロードの最大サイズに制限があります。
ペイロードが大きくなるジョブを扱う場合は、超過分をキャッシュストアに保存し、SQSにはポインターのみを送る設定を追加します。
...
- `enabled` を有効にすると、**1MB以上（at least 1 MB）**のペイロードを指定したキャッシュストアへ保存します。

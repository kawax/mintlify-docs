...
[Laravel Sail](https://github.com/laravel/sail) は、Laravel の Docker 開発環境を操作するための軽量なコマンドラインインターフェースです。
PHP、MySQL、Redis を使った Laravel アプリケーションを、Docker の事前知識がなくても構築できます。
...
<Warning>
  **Laravel 13 では Sail は標準の開発環境ではなくなりました。**
  スケルトンの `composer.json` から `laravel/sail` が削除され、代わりに `composer setup` コマンドが用意されています。
  標準はローカルの PHP + SQLite で動かす構成です。
  ...
  Docker コンテナが必要な場合は、引き続き Sail をインストールして使うことができます。
</Warning>

...
description: "OpenAI・Anthropic・Geminiなど複数のAIプロバイダーを統一インターフェースで扱えるLaravel AI SDKの完全ガイド。エージェント、画像生成、音声、埋め込み、テストまで網羅します。"
---
...
| 機能 | 対応プロバイダー |
|---|---|
| テキスト生成 | OpenAI, Anthropic, Gemini, Azure, Bedrock, Groq, xAI, DeepSeek, Mistral, Ollama, OpenRouter |
| 画像生成 | OpenAI, Gemini, xAI, Azure, Bedrock, OpenRouter |
| 音声合成（TTS） | OpenAI, ElevenLabs, Gemini |
| 音声認識（STT） | OpenAI, ElevenLabs, Mistral, Gemini |
...
flowchart TD
    ...
    B --> C["オプション付与"]
    C -->|"Conversational の場合"| D["会話履歴を付与"]
    C -->|"HasTools の場合"| E["ツール一覧を付与"]
    C -->|"HasStructuredOutput の場合"| F["JSONスキーマを付与"]
    D --> G["AI プロバイダーへ送信"]
    E --> G
    F --> G
    B --> G

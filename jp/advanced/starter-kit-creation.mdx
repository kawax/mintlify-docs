...
- [--using オプション定義](https://github.com/laravel/installer/blob/master/src/NewCommand.php)
- [`create-project` と `post-root-package-install` の実行](https://github.com/laravel/installer/blob/master/src/NewCommand.php)
- [`getStarterKit()` の実装](https://github.com/laravel/installer/blob/master/src/NewCommand.php)

## フロントエンドフレームワークの選択

Laravel 13 の公式スターターキットは複数の選択肢を持ちます。あなたのスターターキットでも、対象を明確に決めて依存を固定してください。

| 選択肢 | 主な構成 | 向いているケース |
| --- | --- | --- |
| React | Inertia 3 + React + Tailwind | ReactでSPAライクに構築したい |
| Vue | Inertia 3 + Vue + Tailwind | Vue中心のチームで開発したい |
| Svelte | Inertia 3 + Svelte + Tailwind | Svelteの軽量性を活かしたい |
| Livewire | Livewire 4 + Blade + Flux UI | PHP中心でインタラクティブUIを作りたい |

<Info>
  どれを選んでも認証のバックエンド方針は共通です。差分は主にフロントエンド構成と初期コンポーネント設計です。
</Info>

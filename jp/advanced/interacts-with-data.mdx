...
`Request::get()` は Symfony 互換のために `Request` クラス本体で提供される別経路のメソッドです。新規コードでは `input()` の利用が推奨です。
...
```mermaid
flowchart TD
    IWD["InteractsWithData<br>共通データアクセスAPI"] --> IWI["InteractsWithInput"]
    IWI --> REQ["Request<br>request()/input()/has()/only()/except()"]
    IWD --> VIN["ValidatedInput<br>safe()/validated()"]
    IWD --> FLU["Fluent"]
    IWD --> UQS["UriQueryString"]
    IWD --> CAB["ComponentAttributeBag"]
    REQ -.関連API.-> SES["Session Store<br>has()/get()/only()/except() を独自実装"]
    REQ -.関連領域.-> VAL["ValidatesAttributes<br>バリデーション判定ロジック"]
```

...
timeline
    ...
    2021 Laravel 8 : Breeze 登場（Fortify なし）
               : シンプルな認証スキャフォールドとして独立
    2024 スターターキット刷新 : React/Vue スターターキットに 2FA 対応が必要に
                         : Fortify が再採用 → 認証全体が Fortify ベースに
    2025 パスキー対応 : Fortify に Passkeys 機能が追加
```
...
<Info>
  Fortify は現在、最も長く使われ続けている Laravel 公式認証パッケージです。Laravel 8 の 2020 年登場以来、Jetstream → Breeze（一部）→ 現行スターターキットと形を変えながら現役を続けています。
</Info>

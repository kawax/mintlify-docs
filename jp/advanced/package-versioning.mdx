...
Laravelはメジャーリリースの数週間前からRC（リリース候補）やbetaを公開します。本番リリース前に動作確認することで、ゼロデイ対応が可能になります。
...
`minimum-stability` を `RC` に設定することで、RC版との依存関係を解決できます。
...
<Tip>
  `prefer-stable: true` を指定すると、安定版が存在する場合は安定版が優先されます。テスト目的でRCを使いながらも、安定版への自動切り替えが保証されます。
</Tip>
...
| 情報源 | 内容 |
|--------|------|
| [laravel/framework CHANGELOG](https://github.com/laravel/framework/blob/master/CHANGELOG.md) | 詳細な変更履歴 |
| [Laravel公式ブログ](https://laravel.com/blog) | メジャーリリースの発表と新機能解説 |
| [laravel/framework リリース](https://github.com/laravel/framework/releases) | タグとリリースノート |

...
Laravelアプリケーションの起動時、サービスプロバイダーとApplicationのフックは以下の順番で実行されます。

1. すべてのServiceProviderの`register()`実行
2. Applicationの`registered()`
3. Applicationの`booting()`
4. すべてのServiceProviderの`boot()`実行
5. Applicationの`booted()`

`AppServiceProvider`に次のコードを追加すると実行順を確認できます。
...
`ApplicationBuilder`の`registered()`、`booting()`、`booted()`はApplicationにコールバックを登録するだけのメソッドです。通常は使う必要がありませんが、起動準備が完了したKernelに対して`booted()`で変更を加えるといった特殊な操作が可能です。
...
<Card title="新アプリ構造 FAQ" icon="circle-question" href="/jp/advanced/app-structure-faq">
  新しいアプリケーション構造に関するよくある疑問と回答をまとめています。
</Card>

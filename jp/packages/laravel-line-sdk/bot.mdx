...
Webhookイベントを受信するとLaravelイベントがディスパッチされます。イベントディスカバリーを有効にするには、`EventServiceProvider` に `shouldDiscoverEvents()` を追加します。
...
`Bot::reply()` を使うと、リプライトークンを渡してメッセージを送信できます。LINEのリプライは自由なため、コントローラーの外側でも使えます。
...
`AppServiceProvider@register` でシングルトンとして登録します。

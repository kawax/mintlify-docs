...
```mermaid
flowchart TD
    A["Laravel Octane"] --> B["FrankenPHP<br>（推奨）"]
    A --> C["RoadRunner<br>（Go製）"]
    A --> D["Swoole<br>（PHP拡張）"]
    B --> E["Go製・追加インストール不要<br>HTTP/3・早期ヒント対応"]
    C --> F["Go製バイナリ<br>シンプルな設定"]
    D --> G["並行タスク・テーブル<br>Tick/Interval対応"]
```
...
<Info>
  [Laravel Cloud](https://cloud.laravel.com) は FrankenPHP を使った Octane をフルマネージドでサポートしています。サーバー設定や Supervisor の管理が不要で、すぐに Octane の恩恵を受けられます。
</Info>

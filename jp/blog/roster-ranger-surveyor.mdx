...
graph LR
    ...
    B --> D["ドキュメント生成や解析ツール"]
    C["laravel/roster<br>依存パッケージ検出"] --> D
    C --> E["laravel/boost"]
```
...
$roster = Roster::scan('/path/to/project');
...
$ranger->onRoute(fn (Components\Route $route) => $routes[] = $route->uri());
$ranger->onModel(fn (Components\Model $model) => $modelAttributes[] = $model->getAttributes());

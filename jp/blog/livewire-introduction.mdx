---
title: "Livewire 3入門 — JavaScriptなしでリアクティブなUIを作る"
description: "LaravelのファーストパーティパッケージLivewire 3を使って、JavaScriptを書かずにインタラクティブなUIを構築する方法を、インストールから実践的なコード例まで解説します。"
---
...
<Info>
  Livewire 3はLaravel 10以降、PHP 8.1以降で動作します。現在の最新バージョンはLivewire 3です。
</Info>
...
デフォルトでは入力のたびにサーバーへリクエストが送られます。リクエストを減らしたい場合は修飾子を使います。
...
Livewire 3では `#[Validate]` アトリビュートでプロパティにバリデーションルールを直接定義できます。

...
### スターターキット生成ツール（Maestro）

[Laravel Maestro](https://github.com/laravel/maestro) はrosterを使って、スターターキット生成前にプロジェクトのパッケージ構成を把握しています。「このプロジェクトはInertiaを使っているか？」「JavaScriptパッケージマネージャーは何か？」という情報を元に、適切なスターターキットのバリアントを選択します。
...
これらのパッケージは主にパッケージ開発者・ツール作者を対象としており、エンドユーザーが直接使うものではありません。しかし、Laravel MaestroのようにLaravel公式ツールの内部でも積極的に活用されており、今後のエコシステム拡大が期待されます。

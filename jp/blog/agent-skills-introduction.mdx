...
```mermaid
graph TD
    A["laravel/agent-skills<br>（公式スキルリポジトリ）"] --> B["laravel-simplifier<br>Laravelコード改善エージェント"]
...
**laravel-simplifier** は、最近変更されたPHP/Laravelコードをレビューし、機能を変えずにコードの明瞭さ・一貫性・保守性を高めるエージェントです。リポジトリ内では `laravel/` ディレクトリに格納されており、インストール時は `laravel-simplifier@laravel` と指定します。
